...
/**
 * Upload a base64 data-URL attachment (image OR short audio recording) to
 * Supabase Storage, for support-chat attachments. Returns { url, type } or
 * null if the data is empty/invalid/too large.
 */
export async function uploadChatAttachment(dataUrl, pathPrefix) {
  if (!dataUrl || typeof dataUrl !== 'string' || !dataUrl.startsWith('data:')) return null;

  const match = dataUrl.match(/^data:((image\/(jpeg|png|webp))|(audio\/(webm|mpeg|mp3|ogg|wav)));base64,(.+)$/);
  if (!match) return null;
  ...
  const ext = extMap[mime] || (isImage ? 'jpg' : 'webm');

  const base64Data = dataUrl.split(',')[1];
  const buffer = Buffer.from(base64Data, 'base64');

  const maxBytes = isImage ? 5 * 1024 * 1024 : 8 * 1024 * 1024; // 5MB images, 8MB voice notes
