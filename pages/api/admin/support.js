...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // One query for ALL unread messages across every ticket, instead of a
    // separate query per ticket — this was the main cause of the admin panel
    // (and Support Chat especially) feeling slow as the ticket list grew.
    const ticketIds = (tickets || []).map((t) => t.id);
    let unreadCountByTicket = {};
    if (ticketIds.length > 0) {
      ...
      (unreadRows || []).forEach((r) => {
        unreadCountByTicket[r.ticket_id] = (unreadCountByTicket[r.ticket_id] || 0) + 1;
      });
    }
    const withUnread = (tickets || []).map((t) => ({ ...t, unread: unreadCountByTicket[t.id] || 0 }));
